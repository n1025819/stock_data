...
class stock(object):
	...
	def get_close(self,end=time_now): #获取当日收盘价
		data_list=self.get_data(end,end)
		return data_list[0].split(',')[3]

	def get_buy_price(self,date_open=time_now): #获取建仓日开盘价
		data_list=self.get_data(date_open,date_open)
		return data_list[-1].split(',')[6]

	def get_highest_profit(self,start='20180101',end=time_now):
		data_list=self.get_data(start,end)
		highest=self.highest_price(start,end)
		tclose=self.get_close(end)
		buy_price=self.get_buy_price(start)
